...
void MSATuioSenderCPP::update() {
	...
	//printf("MSATuioSenderCPP::update");
	...
	for(int i=0; i<OF_MAX_TOUCHES; i++) {
		...
		if(myCursor[i].isAlive && !myCursor[i].wasAlive) {
			if(verbose) printf("MSATuioSenderCPP - addTuioCursor i:%i x:%f, y:%f\n", i, x, y);
			tuioCursor[i] = tuioServer->addTuioCursor(x,y);	
			
		} else if(!myCursor[i].isAlive && myCursor[i].wasAlive) {
			if(verbose) printf("MSATuioSenderCPP - removeTuioCursor i:%i x:%f, y:%f\n", i, x, y);
			...
			if(verbose) printf("MSATuioSenderCPP - updateTuioCursor i:%i x:%f, y:%f\n", i, x, y);
